...
const UserSetting = () => {
    ...
    const displayName = user?.username || user?.email || "Sign In";

    // Handle Logout
    function HandleLogout() {
            userContext.setAuth({ accessToken: null, refreshToken: null, user: null });
    ...
    return (
        <div className="user-setting relative">
            <button className="user-info flex items-center gap-2 cursor-pointer" onClick={() => user ? setOpenDropdown(!openDropdown) : nav("/login")}>
                    <img src={user?.profile_picture || avatar} alt="User Avatar" className="rounded-full w-12 h-12 oblect-cover border-2 border-white" />
            ...
            <ul className={`dropdown-menu ${openDropdown ? 'block' : 'hidden'} w-[300px] bg-white py-5 rounded-2xl shadow-lg absolute top-14 right-0 z-10`}>
                <div className="info p-4 text-center border-b border-gray-300">
                    <img src={user?.profile_picture || avatar} alt="User Avatar" className="rounded-full w-12 h-12 oblect-cover border-2 border-white mx-auto mb-4" />
                    <h3 className="text-md font-semibold">{displayName}</h3>
