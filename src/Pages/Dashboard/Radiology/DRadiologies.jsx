  ...
  useEffect(() => {
    const fetchLabs = async () => {
      try {
        ...
        setLabs(mockRadiology);
      } catch (err) {
        console.error(err);
        setError("Failed to load Radiologies. Please try again.");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-gray-600 text-lg">Loading Radiologies...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="divide-y divide-gray-200">
            {labs.length === 0 ? (
              <div className="px-6 py-12 text-center text-gray-500">
                ...
                <p className="text-lg font-medium">No Radiologies found</p>
                <p className="text-sm mt-1">
                  Add Radiologies to see them appear here
                </p>
              </div>
            ) : (
              labs.map((lab) => (
        ...
        {labs.length > 0 && (
          <div className="mt-4 text-sm text-gray-600 text-center">
            Showing {labs.length}{" "}
            {labs.length === 1 ? "laboratory" : "Radiologies"}
          </div>
        )}
      </div>
    </div>
...
export default RadiologiesDashboard;
